/*
 * mpi_train.cpp  —  Distributed One-vs-Rest linear SVM training (MPI + Adam)
 *
 * Purpose:
 *   Train 4 binary OvR linear SVMs on CICIDS2017 using distributed SGD with
 ...
 *   4. Each rank maintains identical W[4×52] and b[4], initialised to 0.
 *   5. Per epoch:
 *        a. Sample BATCH_SIZE indices (with replacement) from local partition.
 *           Seed: rank*1000 + epoch.
 *        b. Hinge-loss gradient for each OvR classifier + L2 penalty.
 ...
 *   7. Rank 0 writes model.bin and training_log.csv.
 *
 ...
 *   model.bin          — SVM model: int32 N_CLASSES, int32 N_FEATURES,
 *                        float W[4*52], float b[4]
 *   training_log.csv   — epoch,lr,train_loss,val_macro_f1
 */
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        printf("[%s] Data shuffled (seed=42) — each rank receives a balanced class mix.\n",
               ts().c_str());
    }
    ...
    if (rank == 0) {
        printf("[%s] Partition: base=%d/rank, remainder=%d on rank 0  "
               "(rank 0 local_n=%d)\n",
               ts().c_str(), base_n, remainder, local_n);

        // Show class distribution in rank 0's local partition (diagnostic).
        // If macro-F1 looks artificially low during training, it is because
        // rank 0's slice of the data may be class-imbalanced.
        long cls_cnt[N_CLASSES] = {};
        for (int i = 0; i < local_n; ++i)
            if (y_local[i] >= 0 && y_local[i] < N_CLASSES)
                cls_cnt[y_local[i]]++;
        printf("[%s] Rank-0 partition class counts: "
    ...
    std::vector<float> dW_local (N_CLASSES * N_FEATURES);
    std::vector<float> db_local (N_CLASSES);
    std::vector<float> dW_global(N_CLASSES * N_FEATURES);
    std::vector<float> db_global(N_CLASSES);
    ...
    // ══════════════════════════════════════════════════════════════════════
    // PHASE 3 — DISTRIBUTED SVM TRAINING LOOP (One-vs-Rest, Adam+SGD)
    //
    ...
    for (int epoch = 0; epoch < EPOCHS && !stop_flag; ++epoch) {
        ...
        for (int bi = 0; bi < BATCH_SIZE; ++bi) {
            ...
            for (int k = 0; k < N_CLASSES; ++k) {
                // SVM score for class k (One-vs-Rest)
                float score = b[k];
                for (int f = 0; f < N_FEATURES; ++f)
                    score += W[k * N_FEATURES + f] * x[f];

                // Binary label: +1 if this sample belongs to class k, else -1
                int   yk     = (y_true == k) ? 1 : -1;
                float margin = (float)yk * score * CLASS_WEIGHTS[k];

                // Hinge gradient (only when margin < 1)
                if (margin < 1.f) {
        ...
        if (rank == 0 && (epoch + 1) % VAL_INTERVAL == 0) {
            ...
            for (int i = 0; i < local_n; ++i) {
                ...
                int best_k = 0;
                float best_s = b[0];
                for (int f = 0; f < N_FEATURES; ++f) best_s += W[f] * x[f];
    ...
    if (rank == 0) {
        ...
        printf("[%s] Done. Run mpi_infer to apply the hybrid SVM+DBSCAN pipeline.\n",
               ts().c_str());
    }
